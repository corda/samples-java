...
// * State *
// *********
@BelongsToContract(ChainMailSessionContract.class)
public class ChainMailSessionState implements ContractState, LinearState {
    ...
    private Party issuer; // issuer is the creator of the session
    private Party owner; // owner is the receiver of the  session


    // Note Corda always needs a constructor for deserialization in java!
    @ConstructorForDeserialization
    public ChainMailSessionState(UniqueIdentifier linearId, List<String> playerNames, List<String> playerEmails, LinkedHashMap<String, String> assignments, Party issuer, Party owner) {
    ...
    public ChainMailSessionState(List<String> playerNames, List<String> playerEmails, Party issuer, Party owner) {
        this.playerNames = playerNames;
        this.playerEmails = playerEmails;
        this.issuer = issuer;
        this.owner = owner;
        ...
        if (playerNames.size() < 3) {
            throw new IllegalArgumentException("Too few players: " + playerNames.size() + " " + playerNames);
        }
    ...
    public void setAssignments(LinkedHashMap<String, String> assignments) {
        ...
        for (String mail_candidate: playerNames) {

            // ensure all these players exist
            if(!playerNames.contains(mail_candidate)) {
                throw new IllegalArgumentException("A candidate does not exist in the list of players");
            }

            for (String target_candidate: playerNames) {

                // skip duplicates in iteration
                if (mail_candidate.equals(target_candidate)) { continue; }

                // ensure no one is assigned themselves
                if(mail_candidate.equals(assignments.get(mail_candidate))){
