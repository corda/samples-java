...
@StartableByRPC
@InitiatingFlow
public class CheckAssignedChainMailFlow extends FlowLogic<ChainMailSessionState> {

    private final UniqueIdentifier santaSessionId;
    private ChainMailSessionState santaSessionState = null;

    public CheckAssignedChainMailFlow(UniqueIdentifier santaSessionId) {
        this.santaSessionId = santaSessionId;
    }

    @Override
    @Suspendable
    public ChainMailSessionState call() throws FlowException {
        ...
        Vault.Page results = getServiceHub().getVaultService().queryBy(ChainMailSessionState.class, queryCriteria);

        if (results.getStates().size() < 1) {
            throw new IllegalArgumentException("No corresponding GameID Found.");
        }

        santaSessionState = (ChainMailSessionState) ((StateAndRef) results.getStates().get(0)).getState().getData();
    ...
    public ChainMailSessionState getSantaSessionState() {
        return santaSessionState;
    }
}
